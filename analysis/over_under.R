...
for (i in 1:nrow(dynamic)) {
  ...
  under_wins <- margin[1:round(lines$over_under/2)]
  over_wins <- margin[round(lines$over_under/2):length(margin)]
  ...
  if (under_diff > min_diff) {
    ...
    if (lines$away_score + lines$home_score < lines$over_under) {
